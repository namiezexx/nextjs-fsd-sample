...
export default function MainContents({ data }: { data: MainContentsResponse }) {
  return (
    <>
        ...
        {data.data.map((obj, index) => (
          <Card key={index} className="rounded-[12px] border-2">
            ...
            <CardContent className="relative w-80 h-96">
              <Image
                src={`/images/image-${obj.contentsId}.jpg`}
                fill
                alt={`image-${obj.contentsId}`}
                className="px-4"
              />
            </CardContent>
            <CardFooter className="flex justify-between mt-4">
              ...
              <Link href={`/product/${obj.contentsId}`}>
